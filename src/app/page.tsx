...
const inputClass = "w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition";
...
END:VEVENT`;
  ...
  return (
    <div className={darkMode ? 'dark' : ''}>
      <div className={`min-h-screen py-8 px-4 ${darkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900' : 'bg-gradient-to-br from-indigo-100 via-white to-purple-100'}`}>
      <div className="max-w-6xl mx-auto">
        <header className="text-center mb-10">
          <div className="flex justify-end mb-4">
            <button
              onClick={() => setDarkMode(!darkMode)}
              className={`p-2 rounded-lg transition-colors ${darkMode ? 'bg-gray-700 text-yellow-400 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
          ...
          <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-2xl mb-4 shadow-lg shadow-indigo-500/30">
            <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
            </svg>
          </div>
          <h1 className={`text-4xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>QR Code Generator</h1>
          <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>Create beautiful, customizable QR codes for any purpose</p>
        </header>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className={`rounded-3xl shadow-2xl p-6 border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
            <div className="mb-6">
              ...
              <div className="grid grid-cols-5 gap-2">
                {qrTypes.map((type) => (
                  <button key={type.id} onClick={() => setQrType(type.id)} className={`p-3 rounded-xl transition-all duration-200 flex flex-col items-center gap-1 ${qrType === type.id ? "bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-lg" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            ...
            <div className="space-y-5">{renderTypeFields()}</div>

            <div className="mt-8 pt-6 border-t border-gray-200">
              <h3 className={`text-sm font-medium mb-4 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Style Options</h3>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  {(["L", "M", "Q", "H"] as ErrorCorrectionLevel[]).map((level) => (
                    <button key={level} onClick={() => setErrorCorrectionLevel(level)} className={`flex-1 py-2 px-3 rounded-lg font-medium text-sm transition ${errorCorrectionLevel === level ? "bg-indigo-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
                      {level}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="file" accept="image/*" onChange={handleLogoUpload} className="w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition" />
                {logo && (
          ...
          {/* Preview Section */}
          <div className={`rounded-3xl shadow-2xl p-6 border flex flex-col ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="p-6 rounded-2xl shadow-xl" style={{ backgroundColor: bgColor }}>
                {qrValue ? (
                  <QRCodeSVG ref={qrRef} value={qrValue} size={280} bgColor={bgColor} fgColor={fgColor} level={errorCorrectionLevel} includeMargin={true} imageSettings={logo ? { src: logo, height: logoSize * 2.8, width: logoSize * 2.8, excavate: true } : undefined} />
                ) : (
                  <div className="w-[280px] h-[280px] flex items-center justify-center text-gray-400 text-center p-4">Enter data to generate QR code</div>
                )}
              </div>
            </div>

            <div className="mt-6 grid grid-cols-2 gap-3">
              <button onClick={downloadPNG} disabled={!qrValue} className="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 px-4 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg disabled:shadow-none">
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[{ icon: "🎨", title: "Custom Colors", desc: "Personalize colors" }, { icon: "🖼️", title: "Logo Support", desc: "Add your brand" }, { icon: "📱", title: "All Devices", desc: "Scan with any phone" }, { icon: "⬇️", title: "High Quality", desc: "PNG & SVG export" }].map((feature, idx) => (
            <div key={idx} className={`rounded-2xl p-4 border text-center shadow-md ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
              <div className="text-2xl mb-2">{feature.icon}</div>
              <div className={`font-medium text-sm ${darkMode ? 'text-white' : 'text-gray-800'}`}>{feature.title}</div>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{feature.desc}</div>
            </div>
          ))}
        </div>

        <footer className={`mt-10 text-center text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          <p>Generated QR codes work with any QR scanner</p>
          <p className="mt-2">
            Developed by <a href="https://www.immaculatedesigns.com.ng" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 font-medium">Immaculate Designs</a>
          </p>
        </footer>
      </div>
      </div>
